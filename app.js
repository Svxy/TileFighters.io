...
console.log(colors.green("[TileFighters] Starting server..."));
...
if (process.env.PORT == undefined) {
    console.log(colors.blue("[TileFighters] No port defined using default (3000)"));
}
...
console.log(colors.green("[TileFighters] Socket started on port " + port));
...
io.sockets.on("connection", function(socket) {
    ...
    console.log(colors.cyan("[TileFighters] Socket connection with id " + socket.id));
    socket.emit("id", {
        id: socket.id
    });
    ...
    socket.on("disconnect", function() {
        try {
            ...
            console.log(colors.cyan("[TileFighters] Player with id " + socket.id + " disconnected"));
        } catch (err) {
            if (debug) {
                throw err;
            }
    ...
    socket.on('changeName', function(data) {
        try {
            if (data.name.length > 64) { // Name is way too long. Kick the player for sending too much data
                console.log(colors.red("[TileFighters] Player with id " + socket.id + " tried to change name to " + data.name + " but it is longer than 64 chars. Disconnecting socket"));
                disconnectSocket(socket.id);
                return;
            }
            ...
            if (player.name != data.name) {
                console.log(colors.cyan("[TileFighters] Player with id " + socket.id + " changed name to " + data.name));
    ...
    socket.on('kthx', function(data) {
        try {
            ...
            if (!(player == undefined)) {
                ...
                console.log(colors.cyan("[TileFighters] Player with id " + socket.id + " is now verified"));
            }
        } catch (err) {
            if (debug) {
                throw err;
    ...
    try {
        ...
        for (let sa in SOCKET_ACTIVITY) {
            ...
            if (SOCKET_ACTIVITY[sa] > MAX_SOCKET_ACTIVITY_PER_SECOND) {
                console.log(colors.red("[TileFighters] Kicked " + sa + " Too high network activity. " + SOCKET_ACTIVITY[sa] + " > " + MAX_SOCKET_ACTIVITY_PER_SECOND + " Messages in 1 second"));
...
setInterval(function() {
    for (let p in PLAYER_LIST) {
        ...
        if (player.joinKickTimeout == 0 || player.afkKickTimeout <= 0) {
            ...
            console.log(colors.red("[TileFighters] Kicked " + player.id + " for inactivity"));
        }
    }
}, 100);
...
async function update() {
    try {
        ...
        console.log(colors.red("[TileFighters] (Warning) Exception thrown during main update loop. " + err));
        if (debug) {
            throw err;
        }
    }
...
console.log(colors.green("[TileFighters] Server started "));
if (debug) {
