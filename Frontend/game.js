...
ctx.font = "30px Arial";
ctx.fillText("Try reloading the page or consult tnyavntoツ゚#5090 on Discord", 600, 330);

ctx.font = "10px Arial";
setTimeout(function() {
    uiDiv.style.height = "0px";
    $("#menuTextDiv").height("50px");
}, 500);
...
function mouseMove(e) {
    ...
    if (e.clientY < window.innerHeight - 100 && uiVisible && !$("#mlock").prop('checked')) {
        unfocus();
        uiVisible = false;
        uiDiv.style.height = "0px";
        $("#menuTextDiv").height("50px");
    } else if (e.clientY > window.innerHeight - 50 && !uiVisible) {
        uiVisible = true;
...
socket.on("newPositions", function(data) {
    ...
    for (let i = 0; i < data.players.length; i++) {
        ...
        if (data.players[i].spawnCooldown < 0) {
            ...
            if (data.players[i].powerupTime > 0) {
                ...
                ctx.stroke();
                ctx.strokeStyle = "rgba(163, 0, 19, " + playerAlpha + ")";
                ctx.beginPath();
            ...
            if (data.players[i].id == id) {
                playerTextColor = "rgba(0, 0, 0, " + playerAlpha + ")";
            } else {
    ...
    if (dead) {
        $("#death").show();
        countdownDiv.innerHTML = "Respawning in " + respawnCooldown;
    } else {
        $("#death").hide();
    }
});
...
try {
    if (getCookie("jsshooter_name") != "") {
        if (getCookie("jsshooter_name").length > 18) {
            console.error("[Warning] Name stored in cookie is too long. resetting to No Named Man");
            setCookie("jsshooter_name", "No Named Man", 360);
        }
